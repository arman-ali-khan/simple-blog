...
const SingleBlog = ({blog}) => {
	...
	useEffect(()=>{
	  ...
	  useEffect(()=>{
		axios.get(`${process.env.NEXT_PUBLIC_API_PRO}/api/comments?id=${blog.id}`)
		.then(res=>{
	...
	<div className="flex flex-col  overflow-hidden rounded">
		...
		<div className="p-6 pb-12 m-4 mx-auto -mt-16 space-y-6 lg:max-w-4xl sm:px-10 sm:mx-12 lg:rounded-md bg-base-200">
			<div className="space-y-2">
				<p className="inline-block text-lg leading-5 font-bold sm:text-2xl md:text-3xl">{blog.title}</p>
				<div className='flex flex-col sm:flex-row items-center gap-3'>
				<p className="text-sm flex items-center gap-3">By {" "} 
					<Link href={`/user/${blog.username}`} className="text-base text-blue-400 hover:underline">{dbUser.fullName}</Link>
					<Link href={'#comments'}>
					Comment {getComments.count}
				</Link>	
				</p>
				
				<div className='flex items-center text-sm gap-3'>
				<p>{moment(blog.date).fromNow()}</p>
				View: <span className='text-teal-500 font-bold'>{blog.view}</span>
				</div>
